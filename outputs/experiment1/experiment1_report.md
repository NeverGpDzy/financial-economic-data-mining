...
| MAE | 0.055438 |
| R² | -0.3486 |
| IC（Spearman 秩相关） | 0.1667 |
| 方向准确率 | 44.44% |
| 测试集样本数 | 9 |

**关于 R² 为负的说明**：R² < 0 表示模型预测精度不及简单均值预测，这在小样本（测试集仅 9 条）时序预测中属正常现象。IC 和方向准确率更能反映模型的实用预测价值。

正向最优预测滞后（SHAP 特征重要性）：5

### 4.4 SHAP 可解释性分析

SHAP 最重要特征：`H3_lag5`。图表见 `outputs/experiment1/shap_dependence.png`。

### 4.5 双向传导验证

| 方向 | MSE | MAE | R² | IC | 方向准确率 | 最优滞后 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| H3→收益率 | 0.006467 | 0.055438 | -0.3486 | 0.1667 | 44.44% | 5 |
| 收益率→H3 | 0.096413 | 0.225847 | -0.4933 | 0.3333 | 100.00% | 4 |

### 4.6 金融反身性分析

双向模型 R² 均为负（正向 -0.3486，反向 -0.4933），说明在当前小样本条件下，两个方向的预测精度均不及简单均值预测。

但从 IC（秩相关）来看，反向 IC=0.3333，正向 IC=0.1667，收益对情绪的方向性引导信号略强。
