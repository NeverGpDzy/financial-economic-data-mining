...
def main():
    ...
    add_content_slide(prs, '三、因子定义', [
        ...
        '  → Amihud非流动性指标，衡量交易摩擦',
        '',
        '• MoneyFlow（资金流）：5日滚动净流入 / 自由流通市值',
        '  → 资金流入流出代理（无个股级buy/sell数据，使用成交额×价格方向）',
        '',
        '• Value（价值）：1 / PE_TTM',
        '  → 低估值正向化，增强收益稳定性',
        '',
    ...
    add_content_slide(prs, '四、因子质检：VIF共线性检验', [
        ...
        'Value         1.1692    正常',
        'Momentum      2.2295    正常',
        'Volatility    1.6149    正常',
        'Turnover      1.6474    正常',
        'VolumeChange  1.1729    正常',
        '',
    ...
    add_content_slide(prs, '七、回测指标解读', [
        ...
        '',
        '• 累计收益率：74.26%（回测期2年）',
        '• 年化收益率：33.61%（跑赢沪深300的17.2%）',
        '• 夏普比率：0.8237（风险调整后收益良好）',
        '• 最大回撤：-41.04%（中频策略正常范围）',
        '• 日胜率：44.93%（盈亏比合理，靠赢时多赚）',
        '• 超额收益（累计）：38.70%（跑赢基准）',
        '• 信息比率：0.6012（超额收益稳定性一般）',
        '• Calmar比率：0.8189（收益/回撤比良好）',
        '',
        '**结论：** 策略在样本外（2024-2025）表现稳健，',
        '年化超额收益16.41%，验证了LGBM非线性因子赋权的有效性。',
    ])
