...
\graphicspath{{../../outputs/final_project/}}
...
\begin{table}[H]
    \centering
    \scriptsize
    \caption{AI 代码审查与修复摘要}
    \label{tab:ai-audit}
    \begin{tabular}{L{0.15\textwidth}L{0.15\textwidth}L{0.26\textwidth}L{0.28\textwidth}}
        \toprule
        模块 & 功能 & 审核要点 & 修复结果 \\
        \midrule
        \shortstack[l]{data\_loader.py\\factors.py} & 数据加载、因子计算、前瞻收益对齐 & 核查月末因子是否只使用历史行情，并按前瞻收益所属月份划分样本，避免 2024-12 信号对应的 2025-01 收益进入训练。 & 已修复并复核，样本划分改为按 \texttt{fwd\_date} 控制。 \\
        \shortstack[l]{quality.py\\main.py} & IC/IR 与共线性质检 & 原质检结果只输出报告，没有真正驱动 LGBM 训练路径。 & 已修复，\texttt{kept\_factors} 接入训练、IC 方向和综合打分。 \\
        \path{model.py} & LGBM 训练与交叉验证 & 原 \texttt{GroupKFold} 不能保证训练月份早于验证月份。 & 已修复为按月份递进的扩展窗口交叉验证。 \\
        \shortstack[l]{backtest.py\\robustness.py} & 回撤触发减仓风控 & 原风控只缩放收益，未扣除降仓与恢复仓造成的额外换手成本。 & 已修复，新增风控换手与风控成本字段，并汇总进月度成本。 \\
        \shortstack[l]{main.py\\report.py} & 提交材料与报告生成 & 原 AI 审核表和交互记录字符串截断，Word 报告缺少 AI 审查内容。 & 已修复并重新生成报告材料。 \\
        \shortstack[l]{backtest.py\\report.py} & 交易执行口径 & 配套数据没有 \texttt{open} 字段，无法严格模拟次日开盘成交。 & 已在报告中说明月末收盘代理口径及其限制。 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{L{0.24\textwidth}L{0.64\textwidth}}
        ...
        节点 & 内容 \\
        \midrule
        需求理解与数据导入 & 读取期末大作业要求与配套数据，确认任务为申万一级行业轮动多因子策略全流程。 \\
        数据与因子核查 & 核对 31 行业、787 个交易日、PB 与行情代码一致，确认动量、估值、资金流和 Z-Score 口径。 \\
        未来函数修复 & 发现按 \texttt{signal\_date} 划分会污染 2025 年样本外收益，改为按 \texttt{fwd\_date} 划分。 \\
        因子质检修复 & 发现 \texttt{kept\_factors} 未进入建模路径，修复为质检结果驱动训练与打分。 \\
        验证方法修复 & 将 \texttt{GroupKFold} 改为训练月早于验证月的扩展窗口验证。 \\
        风控成本修复 & 补充减仓与恢复仓的额外换手成本，并输出最终风控版指标。 \\
        报告与提交修复 & 补全 AI 审核、交互记录和提交材料，重新生成可复核输出。 \\
        \bottomrule
    \end{tabular}
\end{table}
